...
char	*negative_handle(t_printf *pr, char *str)
{
	...
	if (pr->isnegative)
	{
		...
		free(str);
		pr->flags |= PLUS;
		(pr->flags & SPACE) ? pr->flags &= ~SPACE : 0;
		pr->numlen--;
		return (tmp);
	}
	return (str);
...
void	ft_handle_int(t_printf *pr, long long num)
{
	...
	if (!max_wlong_check(pr, num))
		return ;
	str = ((ft_signed(pr->type))) ? ft_lltoa_base(num, pr->base)
	...
	str = ((pr->flags & PLUS || pr->flags & SPACE) && ft_signed(pr->type))
			? ft_handler_plus_space(pr, str) : str;
	str = ((pr->flags & HASH && ft_octohex(pr->type) &&
			!g_isnull) || pr->type == 'p')
...
{
	pr->issigned = (ft_signed(pr->type)) ? 1 : 0;
